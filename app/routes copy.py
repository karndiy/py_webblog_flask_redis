...
def view_post(id):
    ...
    return render_template('post.html', post=post, comments=comments, current_user=g.get('current_user'))
...
def create_post():
    ...
    return render_template('create_post.html', current_user=g.get('current_user'))
...
def edit_post(id):
    ...
    if request.method == 'POST':
        ...
        post.scheduled_at = datetime.strptime(data['scheduled_at'], '%Y-%m-%dT%H:%M') if data.get('scheduled_at') else None
        db.session.commit()
        return redirect(url_for('main.index'))
    return render_template('create_post.html', post=post, current_user=g.get('current_user'))
